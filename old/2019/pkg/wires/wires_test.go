...
func TestAnswer(t *testing.T) {
	...
	assert.Equal(t, 37390, p2, "Part 2")
}
...
func TestFindIntersection(t *testing.T) {
	...
	for _, test := range tcs {
		i, _ := wires.FindIntersections(test.w1, test.w2)
		p := wires.FindNearestIntersection(i)
		assert.Equal(t, test.expected, p.Distance())
	}
}

func TestFindShortestIntersection(t *testing.T) {
	type tc struct {
		w1       string
		w2       string
		expected int
	...
	for _, test := range tcs {
		i, _ := wires.FindIntersections(test.w1, test.w2)
		steps := wires.FindShortestIntersection(i)
